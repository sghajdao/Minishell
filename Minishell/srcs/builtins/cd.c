...
void ft_print_msg(void)
{
	...
	ms1 = ft_strdup("cd: error retrieving current directory: getcwd: ");
	ms2 = ft_strdup("cannot access parent directories: No such file or directory");
	ms3 = ft_strjoin(ms1, ms2);
	free(ms1);
	free(ms2);
	if (!getcwd(NULL, 0))
		printf("%s\n", ms3);
